...
class trivia(BaseCog):
    ...
    async def start_post(self):
        ...
        await guild.get_channel(816212393922658306).send(f"[{time.strftime('%H:%M:%S', time.gmtime(time.time()))}] Il post automatico si avvierà in {str(delta_time.minutes)} minuti...")
