...
def role_check(message, roles):
    for n, role in enumerate(roles):
        ...
        roles[n] = role
    return len(set(message.author.roles).intersection(set(roles))) > 0
...
class Memes(BaseCog):
    ...
    async def on_message(self, message):
        if role_check(message, [454262524955852800, 454268394464870401, 659513332218331155, 720221658501087312, 676408167063879715, 454262729620848641, 793163338007707679]):
            embed = None
