...
class TicketAlert(BaseCog):
  ...
  async def on_message(self, message):
    if message.channel.id == 683363814137266207:
      ...
      embed = discord.Embed(description = f"[`Richiesta presa in carico da {member.name}#{member.discriminator}`]({message.jump_url})", color = discord.Colour.from_rgb(19, 123, 196))
