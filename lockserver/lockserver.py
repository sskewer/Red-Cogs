...
class LockServer(BaseCog):
    ...
    async def lockserver(self, ctx):
        ...
        if everyonePermissions.send_messages == True:
            await ctx.guild.default_role.edit(permissions=perms_on, reason=f"{member.display_name} ha bloccato il server")
            ...
            await ctx.guild.default_role.edit(permissions=perms_off, reason=f"{member.display_name} ha sbloccato il server")
